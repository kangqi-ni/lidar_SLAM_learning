...
public:
    ...
    */
                      ...
                      tf::TransformListener * tf_)
    {
        ...
        try
        {
            ...
            // transform robot_pose to /odom frame and store the transformed pose in odom_pose
     ...
     * @param start_index            本分段第一个激光点在激光帧中的下标
            ...
            int& beam_number)
    {
        ...
        tf::Vector3 t_start = frame_start_pose.getOrigin();
        // Compute the end pose
        tf::Quaternion q_end = frame_end_pose.getRotation();
        tf::Vector3 t_end = frame_end_pose.getOrigin();

        int end_index = start_index + beam_number;
        for (int i = start_index; i < end_index; ++i) {
            // Compute current pose
            double delta_t = double(i- start_index)/double(beam_number-1);
            tf::Quaternion q_c = q_start.slerp(q_end, delta_t);
            tf::Vector3 t_c = t_start.lerp(t_end, delta_t);
            tf::Transform pose_c (q_c, t_c);

            // Compute the point in current frame
            double angle = angles[i];
            double range = ranges[i];
            tf::Vector3 p(cos(angle)*range, sin(angle)*range, 1);

            // Correct the point by transforming to the base frame
            tf::Vector3 p_corrected = frame_base_pose.inverse() * pose_c * p;
            ranges[i] = sqrt(p_corrected[0]*p_corrected[0] + p_corrected[1]*p_corrected[1]);
            angles[i] = atan2(p_corrected[1], p_corrected[0]);
        }
    }
    ...
    */
                           ...
                           tf::TransformListener * tf_)
    {
        ...
        // 5ms来进行分段
        int interpolation_time_duration = 5 * 1000; // 5 * 1000 us = 5 ms
