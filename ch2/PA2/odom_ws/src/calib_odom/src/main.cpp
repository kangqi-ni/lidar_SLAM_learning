...
void Scan2::scanCallBack(const sensor_msgs::LaserScan::ConstPtr &_laserScanMsg)
{
    ...
    Eigen::Vector3d odom_pose = Eigen::Vector3d::Zero();              //激光对应的里程计位姿
...
Eigen::Vector3d  cal_delta_distence(Eigen::Vector3d odom_pose)
{
    ...
    // Compute R_wl
    double angle_last = last_pos[2];
    Eigen::Matrix3d R_wl;
    R_wl << cos(angle_last), -sin(angle_last), 0,
            sin(angle_last), cos(angle_last), 0,
            0, 0, 1;

    // Compute pose in the last frame 
    d_pos = R_wl.transpose() * (now_pos - last_pos);
