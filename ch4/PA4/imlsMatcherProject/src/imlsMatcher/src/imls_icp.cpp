...
{
    // conver pcl 3d pointcloud to eigen 2d pointcloud
...
void IMLSICPMatcher::setTargetPointCloud(std::vector<Eigen::Vector2d> &_target_pcloud)
{
    ...
    if(m_pTargetKDTree == NULL)
    {
        // each 2d point is a column in kdtree database
                                         ...
                                         double& height)
{
    ...
    for (size_t i = 0; i < nearPoints.size(); ++i) {
        Eigen::Vector2d p_prime = x - nearPoints[i];
        double norm = p_prime.norm();
        double weight = exp(-norm*norm/(m_h *m_h));

        projSum += weight * p_prime.dot(nearNormals[i]);
        weightSum += weight;
    }
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    for (const Eigen::Vector2d &point: nearPoints) {
        mu += point;
    }
    mu /= nearPoints.size();
    ...
    for (const Eigen::Vector2d &point: nearPoints) {
        ...
        Eigen::Vector2d p_prime = point - mu;

        sigma += p_prime * p_prime.transpose();
    }
    ...
    // Choose the eigen vector corresponding to the smaller eigenvalue
    Eigen::Vector2d normal;
    if (D(0,0) < D(1,1)) {
        normal = V.col(0);
    }
